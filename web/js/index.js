const app = new Vue({
  ...
  computed: {
    ...
    connectedTime() {
      return (
        "[" +
        this.pad(this.connected.days, 2) +
        ":" +
        this.pad(this.connected.hours, 2) +
        ":" +
        this.pad(this.connected.minutes, 2) +
        ":" +
        this.pad(this.connected.seconds, 2) +
        "]"
      );
    },
    sortedModes() {
  ...
  methods: {
    ...
    getWanIp() {
      return new Promise((resolve) => {
        axios
          .get("http://ip-api.com/json?fields=query,country,isp")
          .then((res) => {
